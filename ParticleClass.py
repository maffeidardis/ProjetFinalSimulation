...
class Dimers(Monomers):
    ...
    def assignRandomDimerPos(self):
        ...
        while dimer_new_index < self.ND and infiniteLoopTest < 10**4:
            ...
            #mono_i and mono_j form a dimer.
            mono_i, mono_j = dimer_new = self.dimer_pairs[dimer_new_index]
            
            #We place one mono_i by generating random coordinates, given the limits.
            x_pos = np.random.uniform(lim_dim_min[mono_i][0], lim_dim_max[mono_i][0])
            y_pos = np.random.uniform(lim_dim_min[mono_i][1], lim_dim_max[mono_i][1])
            self.pos[mono_i] = [x_pos, y_pos]
            
            #We will then check for overlaps:
            overlap = False
            mono_actual = 0
            mono_new = mono_i
            ...
            while (mono_actual < mono_new) and not overlap:
                ...
                #Here we apply the radium condition to analyse if there is not an overlap event:
                if distance_mono_y ** 2 + distance_mono_x ** 2 <= rad_sum ** 2:
                    overlap = True
                mono_actual += 1

            #If we don't have overlap, we can move forward to mono_j:
            if not overlap:
                #generating random position between the bond length and the closest position possible.
                    rad_sum = self.rad[mono_i] + self.rad[mono_j]
                    bond_max = self.bond_length[dimer_new_index] - self.rad[mono_j] 
                    bond_min = self.bond_length[dimer_new_index] + self.rad[mono_j] 
                    
                    #mono_j can be placed one side of mono_i or the other indifferently:
                    x_pos_j_1 = np.random.uniform(x_pos - bond_min/np.sqrt(2), x_pos - rad_sum/np.sqrt(2))
                    x_pos_j_2 = np.random.uniform(x_pos + rad_sum/np.sqrt(2), x_pos + bond_max/np.sqrt(2))                         
                    ...
                    y_pos_j_1 = np.random.uniform(y_pos - bond_min/np.sqrt(2), y_pos - rad_sum/np.sqrt(2))
                    y_pos_j_2 = np.random.uniform(y_pos + rad_sum/np.sqrt(2), y_pos + bond_max/np.sqrt(2))
                    choice=np.random.randint(1,3)
                    if choice == 1:
                        y_pos_j = y_pos_j_1
                    else: 
                    ...
                    #placing mono_j:
                    self.pos[mono_j] = [x_pos_j, y_pos_j]
                    
                    #checking for overlaps:
                    mono_new = mono_j
                    overlap = False
                    mono_actual = 0
                    lim_dim_max_j = lim_dim_max[mono_j]
                    lim_dim_min_j = lim_dim_min[mono_j]
                    
                    #checking overlap with walls.
                    if self.pos[mono_j][0] > lim_dim_max_j[0] or self.pos[mono_j][1] > lim_dim_max_j[1] or self.pos[mono_j][0] < lim_dim_min_j[0] or self.pos[mono_j][1] < lim_dim_min_j[1]:
                        overlap = True
                        
                    #checking it with other monomer:
                    while (mono_actual < mono_new) and not overlap:
                        distance_mono_x = self.pos[mono_new, 0] - self.pos[mono_actual, 0]
                        distance_mono_y = self.pos[mono_new, 1] - self.pos[mono_actual, 1]
                        ...
                        #Here we apply the radium condition :
                        if distance_mono_y ** 2 + distance_mono_x ** 2 < rad_sum ** 2:
                            overlap = True
                        mono_actual += 1
    
                    #If we don't have overlap, we can move forward to the next dimer:
                    if not overlap:
                        dimer_new_index += 1
    ...
    def Dimer_pair_time(self):
        ...
        #Here we will calculate all the variables that we will need to compute t event
        dist_ofBonds = self.bond_length
        delta_distances = self.pos[mono_i] - self.pos[mono_j]
        distances = np.linalg.norm(delta_distances, axis = 1)
        velocities = self.vel[mono_i] - self.vel[mono_j]
        ...
        a = np.linalg.norm(velocities, axis=1)**2
        b = 2*(delta_distances * velocities).sum(1)
        c = distances ** 2 - dist_ofBonds ** 2
        dif_bac = b**2 - 4*a*c
        dt_time = np.where(dif_bac > 0, (-b+np.sqrt(dif_bac))/(2*a), np.inf)
        tmin = np.argmin(dt_time)
        minCollTime = dt_time[tmin]
        collision_disk_1, collision_disk_2 = self.dimer_pairs[tmin]
